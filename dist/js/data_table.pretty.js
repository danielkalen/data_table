...
(function($) {
  ...
  markup = {
    ...
    table: function(arg) {
      ...
      return "<div class='" + DataTable.defaults.baseClass + " alignment---" + alignment + " sortDirection---{{sortDirection}}'> <div class='" + DataTable.defaults.baseClass + "-heading'> <div class='" + DataTable.defaults.baseClass + "-heading-row'> <div class='" + DataTable.defaults.baseClass + "-heading-row-cell _expandButton'></div> </div> </div> <div class='" + DataTable.defaults.baseClass + "-body'></div> </div>";
    },
    loading: function() {
    ...
    paginationItem: function(arg) {
      var value;
      value = arg.value;
      return "<div class='" + DataTable.defaults.baseClass + "-pagination-item _paginationItem'> <div class='" + DataTable.defaults.baseClass + "-pagination-item-text'>" + value + "</div> </div>";
    },
    headingCell: function(arg) {
      var extraClasses, icon, label, ref, ref1, ref2, slug, style;
      extraClasses = (ref = arg.extraClasses) != null ? ref : '', slug = arg.slug, icon = (ref1 = arg.icon) != null ? ref1 : '', label = arg.label, style = (ref2 = arg.style) != null ? ref2 : '';
      return "<div class='" + DataTable.defaults.baseClass + "-heading-row-cell " + extraClasses + " __" + slug + "' data-slug='" + slug + "' data-icon='" + icon + "' " + style + "> <div class='" + DataTable.defaults.baseClass + "-heading-row-cell-text'>" + label + "</div> </div>";
    },
    row: function(arg) {
      var cells, drilldown, ref, rowID;
      rowID = arg.rowID, cells = arg.cells, drilldown = (ref = arg.drilldown) != null ? ref : '';
    ...
    ipDetailsItem: function(arg) {
      var label, value;
      label = arg.label, value = arg.value;
      return "<div class='" + DataTable.defaults.baseClass + "-ipDetails-content-item'> <div class='" + DataTable.defaults.baseClass + "-ipDetails-content-item-label'>" + label + ": </div> <div class='" + DataTable.defaults.baseClass + "-ipDetails-content-item-value'>" + value + "</div> </div>";
    },
    fields: function(arg) {
      var fields;
      fields = arg.fields;
      return "<div class='" + DataTable.defaults.baseClass + "-fieldGroup'>" + fields + "</div>";
    },
    fieldsItem: function(arg) {
      var label, value;
      label = arg.label, value = arg.value;
      return "<div class='" + DataTable.defaults.baseClass + "-fieldGroup-item'> <div class='" + DataTable.defaults.baseClass + "-fieldGroup-item-label'>" + label + ": </div> <div class='" + DataTable.defaults.baseClass + "-fieldGroup-item-value'>" + value + "</div> </div>";
    },
    button: function(arg) {
      var action, icon, isMulti, ref;
      action = arg.action, icon = (ref = arg.icon) != null ? ref : '', isMulti = arg.isMulti;
      return "<div class='" + DataTable.defaults.baseClass + "-button _actionButton " + (isMulti ? '_isMulti' : '') + "' data-action='" + action + "'> <div class='" + DataTable.defaults.baseClass + "-button-icon'>" + icon + "</div> </div>";
    },
    ...
    if (row.processed) {
      ...
      if (this.hasBreakdownBar && row.breakdownBarEl.length) {
        SimplyBind('largestBreakdownTotal').of(this).to('updatedBreakdownWidth').of(row).transform(function() {
          ...
          return function() {
            var drilldownEl, i, index, len, ref, ref1, width;
            ref = row.drilldownEls;
            for (index = i = 0, len = ref.length; i < len; index = ++i) {
              drilldownEl = ref[index];
              width = helpers.getBreakdownBarWidth(row.drilldown[index], row.drilldown.largestBreakdownTotal);
              if ((ref1 = $(drilldownEl).children('.is_breakdown_bar').children().children()[0]) != null) {
                ref1.style.width = width + '%';
              }
            }
          };
        })(this)).conditionAll(function() {
          return row.visible;
        });
      }
      row.processed = true;
  ...
  DataTable.prototype.generateRow = function(row) {
    ...
    if (row.drilldown) {
      ...
      if (this.hasBreakdownBar) {
        row.drilldown.largestBreakdownTotal = Math.max.apply(null, row.drilldown.map(function(subRow) {
      ...
      'fields': (function() {
        ...
        output = (function() {
          ...
          for (label in dataFields) {
            ...
            results.push(markup.fieldsItem({
              label: label,
              value: value
            }));
          }
      ...
      'actions': (function(_this) {
        return function() {
          ...
          output = (function() {
            ...
            for (i = 0, len = ref.length; i < len; i++) {
              action = ref[i];
              results.push(markup.actionsItem(action));
            }
            return results;
          }).call(_this);
          return output.join('');
  ...
  DataTable.prototype.attachEvents = function() {
    ...
    this.els.tableBody.on('click', '._expandButton', (function(_this) {
      return function(event) {
          ...
          return _this.options.ipDataFetcher(ipAddress).then(function(ipDetails) {
            ...
            output = (function() {
              ...
              for (label in ipDetails) {
                ...
                results.push(markup.ipDetailsItem({
                  label: label,
                  value: value
                }));
              }
  ...
  DataTable.prototype.attachBindings = function() {
    ...
    SimplyBind('pageCount').of(this).to('innerHTML').of(this.els.paginationItems).transform(function(count) {
      ...
      for (value = i = 1, ref = count; 1 <= ref ? i <= ref : i >= ref; value = 1 <= ref ? ++i : --i) {
        if (value !== 0) {
          paginationItems += markup.paginationItem({
            value: value
          });
        }
      }
